...
for restart in range(flags.n_restarts):
    ...
    # np.random.shuffle(wjc_data.numpy())
    # np.random.set_state(rng_state)
    # np.random.shuffle(wjc_target.numpy())
    # np.random.set_state(rng_state)
    # np.random.shuffle(this_dataset)
    # np.random.set_state(rng_state)
    # np.random.shuffle(k_labels)

    # Build environments
    # def wjc_make_environment(images_data, targets_data, name_data):
    #     envs = []
    #     division_feature = []
    #     # collect all models of disks
    #     for i in range(len(name_data)):
    #         mid_group = pd.read_csv(main.single_dataset_path + "/" + name_data[i] + '.csv')
    #         if model_to_int(mid_group.loc[:, ['model']].iloc[0, 0]) not in division_feature:
    #             division_feature.append(model_to_int(mid_group.loc[:, ['model']].iloc[0, 0]))
    #     print('different values of segment features:', len(division_feature))
    #     for j in range(len(division_feature)):
    #         mid_feature = division_feature[j]  # mid_feature: current feature filter.
    #         mid_env = []  # mid_env: current disks index pass filter
    #         for i in range(len(name_data)):
    #             mid_group = pd.read_csv(main.single_dataset_path + "/" + name_data[i] + '.csv')
    #             if model_to_int(mid_group.loc[:, ['model']].iloc[0, 0]) == mid_feature:
    #                 mid_env.append(i)
    #
    #         # print(len(mid_env))
    #         mid_env_images = (images_data[torch.tensor(mid_env).long(), :, :].float()).to(device)
    #         mid_env_labels = (targets_data[torch.tensor(mid_env).long(), None]).to(device)
    #         envs.append({"images": mid_env_images, "labels": mid_env_labels})
    #     return envs

    # envs = wjc_make_environment(mnist_train[0], mnist_train[1], mnist_train[2])
    ...
    print('Final test set performance (acc) (Vanilla TCN):')
